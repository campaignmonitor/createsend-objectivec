...
NSString *const kCSSSKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const kCSSSKeychainAccountKey = @"acct";
NSString *const kCSSSKeychainCreatedAtKey = @"cdat";
NSString *const kCSSSKeychainClassKey = @"labl";
NSString *const kCSSSKeychainDescriptionKey = @"desc";
NSString *const kCSSSKeychainLabelKey = @"labl";
NSString *const kCSSSKeychainLastModifiedKey = @"mdat";
NSString *const kCSSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef CSSSKeychainAccessibilityType = NULL;
#endif

@interface CSSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation CSSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = CSSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kCSSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (CSSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return CSSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (CSSSKeychainAccessibilityType) {
		CFRelease(CSSSKeychainAccessibilityType);
	}
	CSSSKeychainAccessibilityType = accessibilityType;
}
#endif
